...
{
   primera_libre = 0 , primera_ocupada = 0;
   ocupadas      = newCondVar();
   libres        = newCondVar();
   cont_libres = num_celdas_total;
   cont_ocupadas = 0;
}
// -----------------------------------------------------------------------------
...
int ProdConsMU1::leer(  )
{
   ...
   assert( 0 < cont_ocupadas );

   const int valor = buffer[primera_ocupada] ;
   
   // hacer la operación de lectura, actualizando estado del monitor
    if (primera_ocupada < num_celdas_total-1)
        primera_ocupada++;
...
{
   // esperar bloqueado hasta que primera_ocupada > 0
   if ( cont_libres == 0 )
      libres.wait();
     
   
   // cout << "escribir: ocup == " << primera_libre << ", total == " << num_celdas_total << endl ;
   assert( 0 < cont_libres  );
...
{
   cout << "--------------------------------------------------------------------" << endl
        << "Problema del productor-consumidor múltiples (Monitor MU, buffer FIFO). " << endl
        << "--------------------------------------------------------------------" << endl
        << flush ;
