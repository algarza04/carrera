//c++ -std=c++11 -pthread -o prodcons_mu_fifo prodcons_mu_fifo.cpp scd.cpp scd.h
// -----------------------------------------------------------------------------
//
...
constexpr int
   ...
   p = num_items / np , //número de items a producir por cada productor
   c = num_items / nc ; // número de items a consumir por cada consumidor
int
   siguiente_dato[np] = {0} ; // siguiente valor a devolver en 'producir_dato'
...
int producir_dato( int nhebra )
{
   ...
   const int valor_producido = nhebra*p + siguiente_dato[nhebra] ;
   siguiente_dato[nhebra] ++ ;
   mtx.lock();
...
void consumir_dato( unsigned valor_consumir, int nhebra )
{
   ...
   this_thread::sleep_for( chrono::milliseconds( aleatorio<min_ms,max_ms>() ));
   mtx.lock();
   cout << "                  hebra consumidora " << nhebra << ", consume: " << valor_consumir << endl ;
   mtx.unlock();
}
//----------------------------------------------------------------------
...
class ProdConsSU1 : public HoareMonitor
{
 ...
 int                        // variables permanentes
   ...
   primera_ocupada ,			//indice de la celda de la próxima extracción
...
void funcion_hebras_productoras( MRef<ProdConsSU1> monitor, int nhebra )
{
...
void funcion_hebras_consumidoras( MRef<ProdConsSU1>  monitor, int nhebra )
{
   for( unsigned i = 0 ; i < c ; i++ )
   {
      int valor = monitor->leer();
      consumir_dato( valor, nhebra ) ;
   }
}
// -----------------------------------------------------------------------------
...
int main()
{
   ...
   thread hebras_prods[np],
          hebras_cons[nc];

	for (int i = 0; i < np; i++)
		hebras_prods[i] = thread( funcion_hebras_productoras, monitor, i);
