...
class modeloGLTF:
    ...
    def flotar(self):
        ...
        if bbox is None:
            print("Modelo sin geometría, no se puede flotar.")
            return
...
class escenaPYGFX:
    ...
    def ilumina_modelo(self, modelo, intensidad=0.5):
        sphere = modelo.model_obj.get_world_bounding_sphere()
        if sphere is None or len(sphere) < 4:
            print("Modelo sin esfera de bounding válida, no se puede iluminar.")
            return
