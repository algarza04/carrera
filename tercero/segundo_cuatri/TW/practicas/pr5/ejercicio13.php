...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['envio'])){
    if (empty($_GET['nombre'])){
        ...
        $datos['nombre'] = strip_tags($_GET['nombre']);
    }

    if (!empty($_GET['apellidos'])){
        $datos['apellidos'] = strip_tags($_GET['apellidos']);
    } 
    ...
    if (empty($_GET['fecha_nacimiento'])) {
        ...
        $datos['fecha_nacimiento'] = strip_tags($_GET['fecha_nacimiento']);
    }

    if (!empty($_GET['nacionalidad'])) {
        $datos['nacionalidad'] = strip_tags($_GET['nacionalidad']);
    }

    if (empty($_GET['sexo']) || !in_array($_GET['sexo'], ['Masculino', 'Femenino', 'NA'])) {
        $errores['sexo'] = 'Seleccione un sexo válido';
    } else {
        $datos['sexo'] = strip_tags($_GET['sexo']);
    }
    ...
    if (!empty($_GET['datos_almacenados']) && in_array($_GET['datos_almacenados'], ['TOTAL', 'PARCIAL', 'NINGUNO'])) {
        $datos['datos_almacenados'] = strip_tags($_GET['datos_almacenados']);
    } 
...
function Checked($value, $expected) {
    return (htmlspecialchars($value) === htmlspecialchars($expected)) ? 'checked' : '';
}

function CheckedCheckbox($value) {  //esta función es para los checkbox ya que con la anterior no funciona como esperamos
    return htmlspecialchars($value) ? 'checked' : '';
}

function Selected($value, $expected) {
    return  htmlspecialchars($value) ===  htmlspecialchars($expected) ? 'selected' : '';
}
...
<body>
    ...
    <?php if (!empty($datos['nombre'])) echo "<p>Bienvenido/a ${datos['nombre']}, siga instrucciones a continuación:</p>"; ?>
    <form action="" method="GET" novalidate> 
        <section class="datos_personales">
            ...
            <div>
                <div class="alinear_datos">
                    <label>Nombre 
                        <input type="text" name="nombre" size="20" value="<?=  htmlspecialchars($datos['nombre']) ?>" required placeholder="El nombre es obligatorio" <?= $readonly ?>>
                    </label>  
                    <span class="error"><?= $errores['nombre'] ?? '' ?></span>
                    <label>Apellidos
                        <input type="text" name="apellidos" size="30" value="<?=  htmlspecialchars($datos['apellidos']) ?>" placeholder="Esto es opcional" <?= $readonly ?>>
                    </label>
                </div>
                <div class="alinear_datos">
                    <label>DNI
                        <input type="text" name="dni" value="<?=  htmlspecialchars($datos['dni']) ?>" required size="9" <?= $readonly ?>>
                    </label>
                    <span class="error"><?= $errores['dni'] ?? '' ?></span>
                    <label>Fecha de nacimiento
                        <input type="date" name="fecha_nacimiento"  required value="<?=  htmlspecialchars($datos['fecha_nacimiento']) ?>" <?= $readonly ?>>
                    </label>
            ...
            <div>
                <div class="alinear_datos">
                    <label>Nacionalidad
                        <input type="text" name="nacionalidad" value="<?=  htmlspecialchars($datos['nacionalidad']) ?>" <?= $readonly ?>>
                    </label>
                    <label>Sexo
        ...
        <section class="datos_acceso">
            ...
            <div>
                <label>E-mail
                    <input type="email" name="email" size="30" value="<?=  htmlspecialchars($datos['email']) ?>" required <?= $readonly ?>>
                </label>
                <span class="error"><?= $errores['email'] ?? '' ?></span>
                <label>Clave
                    <input type="password" name="clave" size="15" value="<?=  htmlspecialchars($datos['clave']) ?>" required placeholder="Escriba la clave" <?= $readonly ?>>
                </label>
                <span class="error"><?= $errores['clave'] ?? ''?></span>
                <label>(repítala) 
                    <input type="password" name="clave_repetida" size="15" value="<?=  htmlspecialchars($datos['clave_repetida']) ?>" required <?= $readonly ?>>
                </label>
